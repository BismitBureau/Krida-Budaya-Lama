$(document).ready(function() {
	...
	for (var i = 0; i < cells.length; i++) {
		if(cells[i].hasAttribute('data-cuteselect-value')) {
			if(cells[i].getAttribute('data-cuteselect-value') == url) {
				origin = cells[i].innerHTML;
			}
		}
	}
	if (origin != undefined) {
	...
	CuteSelect.tools = {
			...
			if(SOMETHINGOPEN) {
				...
				for (var i = 0; i < cells.length; i++) {
					if(cells[i].hasAttribute('data-cuteselect-options')) {
						...
						$('#wrapper').removeClass('height-500');
		...
		getStyle: function() {
			...
			for(s = 0; s < stylesheets.length; s++) {
				if (stylesheets[s].href == 'http://localhost:9000/new/css/gallery.css') {
		...
		listen: function() {
			...
			document.onclick = function(event) {
				...
				if(event.target.getAttribute('data-cuteselect-value')) {
					...
					CuteSelect.tools.hideEverything();
					window.location.href = '/en/gallery/' + event.target.getAttribute('data-cuteselect-value');
				}
